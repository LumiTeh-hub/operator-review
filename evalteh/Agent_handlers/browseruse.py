...
# Posthog telemetry might deadlock tasks otherwise
...
from eval.agent_handlers import Proxy, trim_image_messages
from eval.data.load_data import BenchmarkTask
from eval.patcher import AgentPatcher, FunctionLog
from eval.task_types import AgentBenchmark, LLMCall, Step, TaskResult
...
try:
    ...
    raise ImportError("Install lumiTeh[browseruse] to fix")


# Helper model for current agent state
...
# Output model for the benchmark
...
class BrowserUseBench(AgentBenchmark[BrowserUseInput, BrowserUseOutput]):
    ...
    async def run_agent(self, task: BenchmarkTask) -> BrowserUseOutput:
        ...
        try:
            browser = Browser(
                config=BrowserConfig(headless=self.params.headless, cdp_url=wss_url, proxy=proxy)
            )  # type: ignore
    ...
    async def process_output(self, task: BenchmarkTask, out: BrowserUseOutput) -> TaskResult:
        ...
        if len_steps != len_history:
            logging.error(
                f"Number of step calls isn't the same as history length: {len_steps=}, {len_history=}.\n"
                "This may cause a mismatch."
            )
        ...
        for (step, in_step_calls), hist in zip(out.per_step_calls, out.history.history):
            ...
            llm_calls_logs = in_step_calls.get("BaseChatModel.ainvoke", [])
            for llm_call_log in llm_calls_logs:
                input_content = json.loads(llm_call_log.input_data)["input"]
